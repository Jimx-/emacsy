...
(define (parse-entry lang docstrings options def-line filename line-no)
  ...
  (cond
   (def-line
     (let ((type (cond ((equal? lang 'c) 'c-function)
                       ...
                       ((string-match "\\(define-syntax" def-line) 'macro)
                       ((string-match "\\(define-class(-public|) " def-line) 'class)
                       ((string-match "\\(define-record" def-line) 'record)
                       (else 'procedure))))
       (make-entry type
                   (get-symbol def-line)
  ...
  (define (->string s)
    (cond ((null? s) "")
          ...
          ((number? s)
           (number->string s))
          ((keyword? s)
           (string-append " #:" ((compose symbol->string keyword->symbol) s)))
          ((boolean? s)
           (if s "#t" "#f"))))
  (cond ((null? s)
	 "")
...
(define (format-texinfo entry)
  (case (entry-type entry)
    ((procedure)
     ...
     (newline)
     (display "@deffn {Scheme Procedure} ")
    ...
    ((interactive)
     ...
     (let ((signature (entry-signature entry)))
       (display "@deffn {Interactive Procedure} "))
     (display (entry-signature entry))
     (newline)
     (display "@kindex ")
     (display (entry-signature entry))
     ...
     (write-line "@end deffn"))
    ((class)
     (format (current-error-port) "class entry: ~s\n" entry)
     ;; (display "\n\f")
     ;; (display (entry-symbol entry))
     (newline)
     (display "@c snarfed from ")
     ...
     (for-each (lambda (s) (display "@c ") (write-line s))
               (entry-options entry))
     (write-line "@end deftp"))
    ((record)
     ;; (format (current-error-port) "type entry: ~s\n" entry)
     ;; (display "\n\f")
     ;; (display (entry-symbol entry))
     (newline)
     (display "@c snarfed from ")
     ...
     (write-line "@end deftp"))
    ((variable)
     ;; (format (current-error-port) "variable entry: ~s\n" entry)
     ;; (display "\n\f")
     ;; (display (entry-symbol entry))
     (newline)
     (display "@c snarfed from ")
     ...
     (display (entry-line entry))
     (newline)
     (display (if (string-match "^[a-zA-Z]+ [a-z]" (entry-signature entry)) "@deftypevr "
                  "@defvar "))
     (display (entry-signature entry))
     (newline)
     (for-each (lambda (s) (write-line s))
